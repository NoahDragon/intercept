...
{
	sscanf(serialized.c_str(), "%hx,%x,%hx", &stroke.code, &stroke.information, &stroke.state);
}
...
void KeyCombo::Read(unsigned short endcode, bool nosticky)
{
	Interception in(INTERCEPTION_FILTER_KEY_ALL);
	std::vector<unsigned short> combos;
	...
	while (1) {
		...
		if ( k.stroke.code == endcode ) {
			...
			if (nosticky) {
				if (k.IsDown()) {
					if (std::find(combos.begin(), combos.end(), k.stroke.code) != combos.end())
						continue;
					combos.push_back(k.stroke.code);
				}
				else {
					combos.erase(std::remove(combos.begin(), combos.end(), k.stroke.code), combos.end());
				}
			}
			Add(k);
			k.Print();
...
void KeyFilterSet::Print(int i)
{
	...
	if ((unsigned int) i > filters.size() - 1)
		return;
...
int KeyFilterSet::GetFilterNumber(void)
{
	...
	n = n - 1;
	if ((n < 0) || ((unsigned int)n > filters.size() - 1)) {
		printf("\a");
		return -1;
	}
